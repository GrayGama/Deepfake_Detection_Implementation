    ...
    def facecrop(
        ...
        for cnt_frame in range(frame_count_org):
            ...
            if image_path.is_file():
                logger.info(f"Skipping {image_path} as it already exists.")
                continue
...
if __name__ == '__main__':
    ...
    if dataset_name == 'FaceForensics++':
        sub_dataset_path = Path(os.path.join(dataset_path, 'manipulated_sequences', 'NeuralTextures', comp))
        mask_dataset_path = Path(os.path.join(dataset_path, 'manipulated_sequences', 'NeuralTextures', 'masks'))
